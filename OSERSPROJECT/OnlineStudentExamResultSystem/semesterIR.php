...
if(isset($_POST['insertonce'])){
    ...
    if(empty($id) || empty($name)){
        ...
        if(!preg_match("/[a-z A-Z]/",$name) || !preg_match("/[0-9]/",$id)){
            $insertok = false;
        $insertinfo = 'invalid input';
        }
      
    }
            $fq0 = "SELECT * FROM `first_year_rt`"; 
            $res0 = mysqli_query($con,$fq0);
            while($row = mysqli_fetch_array($res0)){
                if($id == $row['St_ID']){
                    $insertok = false;
                   $insertnoti = 'You Cannot insert the student that have already exist';
                }
            }

     if($insertok){
                    $sql = "INSERT INTO `first_year_rt`(`St_ID`, `St_Name`, `M-100`, `E-100`, `P-100`, `CST-101`, `CST-102`, `CST-103`, `GPA`, `Major`) VALUES
                ...
                mysqli_query($con,$delet);
                    ...
                    $insertnoti = 'Invalid input datatype or empty value or Data already exist!Try again!';
                }
            }else{
                $insertnoti='You Don\'t have permission to insert the result';
            }       
    }
...
for($i = 0;$i < $_SESSION['nos'];$i++){
    ...
    while($row = mysqli_fetch_array($res0)){
        if($idv == $row['St_ID']){
            $insertok0 = false;
           $insertinfo = 'You Cannot insert the student that have already exist';
        }
    }
    ...
    if($insertok0){
        ...
        $firstgpa = GPA1($mv,$ev,$pv,$f01v,$f02v,$f03v,$con);
            ...
            $result = mysqli_query($con,$sql);
            if($result){
                $insertinfo = 'your data to first year semester_I have Inserted '.$_SESSION['nos'].' Student';
            }
            else{
                $insertinfo = 'error';
            }
            ...
            $insertinfo = 'You Forget Some Field To Fill or Invalid Input type! Or Data Already Exist!';
        }
...
mysqli_query($con,$delet);
    ...
    $insertinfo = 'You don\'t have permission to insert the result';
}
}else{
    $insertinfo = "";
}
if(isset($_POST['sinsertonce'])){
    ...
    if($sinsertok){
        $sql = "INSERT INTO `second_year_rt`(`St_Name`, `St_ID`, `E-200`, `CST-201`, `CST-202`, `CST-203`, `CST-204`, `CST-205`, `GPA`, `Major`) VALUES
        ('$psname','$psid','$pes','$ss01','$ss02','$ss03','$ss04','$ss05','$gs','$mjs')"; 
        $result = mysqli_query($con,$sql);
        if($result == true){
        ...
        $insertinfo = 'Invalid Input Type or Empty Input or You Have Input Data That have already exist';
    }
    }else{
        $insertinfo = 'You Don\'t have permission to insert result';
    }
}
if(isset($_POST['sinsertall'])){
    ...
    for($i = 0;$i < $_SESSION['nos'];$i++){
        ...
        while($row = mysqli_fetch_array($res)){
            if($idv == $row['St_ID']){
                $insertok1 = false;
                $insertnoti = 'Sorry Data Already Exist!';
            }
        }
        ...
        if($insertok1){
            ...
            $secondgpa = GPA2($mv,$ev,$pv,$f01v,$f02v,$f03v,$majv,$con);
                $sql = "INSERT INTO `second_year_rt`(`St_ID`, `St_Name`, `E-200`, `CST-201`, `CST-202`, `CST-203`, `CST-204`, `CST-205`, `GPA`, `Major`) VALUES
                ('$idv','$nv','$mv','$ev','$pv','$f01v','$f02v','$f03v','$secondgpa','$majv')";
                $result = mysqli_query($con,$sql);
                if($result){
                    $insertinfo = 'your data to second year semester_I have Inserted '.$_SESSION['nos'].' Student';
                }
                else{
                    $insertinfo = 'error';
                }
                ...
                $insertinfo = 'You Forget Some Field To Fill or Invalid Input type! or
                Your input is already exist';
            }
...
$delet = "DELETE FROM `second_year_rt` WHERE St_ID = ''";
mysqli_query($con,$delet);
    }else{
...
if(isset($_POST['tinsertonce'])){
    ...
    $gt = GPA3($pet,$st01,$st02,$st03,$st04,$st05,$mjt,$con);
        ...
        if($insertok2){
        $sql = "INSERT INTO `third_year_rt`(`St_Name`, `St_ID`, `E-300`, `CST-301`, `CST-302`, `CST-303`, `CST-304`, `CST-305`, `GPA`, `Major`) VALUES
        ('$ptname','$ptid','$pet','$st01','$st02','$st03','$st04','$st05','$gt','$mjt')"; 
        $result = mysqli_query($con,$sql);
        if($result == true){
        ...
        $delet = "DELETE FROM `third_year_rt` WHERE St_ID = ''";
        mysqli_query($con,$delet);
}else{
    $insertinfo = 'You don\'t have permission to insert result';
}
    ...
    for($i = 0;$i < $_SESSION['nos'];$i++){
        ...
        if($insertok2){
            ...
            $secondgpa = GPA3($mv,$ev,$pv,$f01v,$f02v,$f03v,$majv,$con);
                $sql = "INSERT INTO `third_year_rt`(`St_ID`,`St_Name`, `E-300`, `CST-301`, `CST-302`, `CST-303`, `CST-304`, `CST-305`, `GPA`, `Major`) VALUES
                ('$idv','$nv','$mv','$ev','$pv','$f01v','$f02v','$f03v','$secondgpa','$majv')";
                $result = mysqli_query($con,$sql);
                if($result){
 ...
 <form action = "studentinsert.php" method = "post">
        ...
        <div id="showinsertpane">   
            <div id="studentchoice">
                <form action="semesterIR.php" method="post">
                <label><strong>Choose The Students Result Table For Semester I</strong></label>
            </div>
...
<option>F</option>
           ...
           <?php 
            ...
            <option>C</option>
            <option>D</option>
            <option>F</option>http://localhost/OSERSPROJECT/OnlineStudentExamResultSystem/panelock.php
            </select></td>
       ...
       <?php              }
           ...
           <tr align = "center"><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><input type ="submit" name ="insertall" value= "Insert All Students" onclick = "return confirm('Are you sure to Insert All Data!')"/></td></tr>
            ...
            <tr><td><strong style = "text-align:center">Insert One Student</strong></td></tr>
            <tr><th>ID</th><th>Name</th><th>E2001</th><th>2011</th><th>2021</th><th>2031</th><th>2041</th><th>2051</th><th>Major</th></tr>
